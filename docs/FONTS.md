...
全站使用 Adobe 思源黑體的臺灣繁中區域版本 **Source Han Sans TW VF**，同時涵蓋繁體中文與拉丁字母。字重範圍為 250–900，保留網站既有標題與內文層級。
...
`src/styles/global.css` 的 `--font-site` 同時指定所有 typography 字型 token，避免英文、引文和程式碼落到另一套字型。原始通用繁中排版規範保留不動。
