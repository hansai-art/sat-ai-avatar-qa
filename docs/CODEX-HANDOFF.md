...
> 2026-09-23：操作圖片採小預覽，點開支援縮放與拖曳。兩張已核對的模型選單／HTTP 429 截圖對應 qa-000052、qa-000053；新增圖片格式及驗收見 [圖片閱讀規格](IMAGE-VIEWER.md)。題庫維持 62 題，原始私人來源不公開。

> 2026-09-22 最新改版：真實題庫目前為 62 題。指定資料夾 Q001 至 Q066 已全數盤點，104 個問題對應項目合併為 62 題，詳見 [全部問答匯入](FAQ-FULL-IMPORT.md)。學生入口、統一六分類、章節／小節分層、三維篩選、結論先行與匿名來源，以 [學生改版紀錄](STUDENT-REVAMP-2026-09-22.md) 為準。下方日期較早的 demo、第一章不分小節與公開學生姓名等敘述屬歷史狀態，不可沿用。


## 2026-09-20 接續：回答品質

Hans 提供好的回答範例後，既有 22 題重新整理判斷、第一步、具體輸入與產出、核對方法。需要時比較替代流程，簡單排錯維持直接回答。後續編輯先讀 [回答品質標準](FAQ-ANSWER-STANDARD.md)，本輪逐題審查與驗收見 [內容改版紀錄](FAQ-ANSWER-REVISION-2026-09-20.md)。假設示範不計入學生提問，來源對照、ID 與常見排序保留。
...
新站公開驗收已完成：`c44cbd97d0601f88817ba956b8a6beb80f3adadf`，40/40 通過（19.9 秒）；本機另核對 Cloudflare github:push、6 組 HTTP 轉址及匿名 360px 手機搜尋／第一章。證據見 ACCEPTANCE.md。日常內容維護不需要反覆修改 Cloudflare 設定。

剩餘主任務：Hans 尚未提供正式問題、提問者與原討論網址；收到後依章節整理、補官方來源、以真實已問題衍生並後排延伸問題，保留姓名和來源，不得捏造。現有 UI 與資料欄位已支援，但正式內容仍未完成。維持 BUILD_MODE=demo，課綱未核對前不將 confirmed 批次改 true。部署設定見 CLOUDFLARE.md。
