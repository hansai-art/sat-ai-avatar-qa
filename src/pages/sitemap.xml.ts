...
export const GET:APIRoute=async({site})=>{
  const urls=buildMode==='production'&&site?.protocol==='https:'?['/','/questions/','/chapters/','/tools/','/types/','/about/',...taxonomy.chapters.map(c=>`/chapters/${c.id}/`),...taxonomy.tools.map(t=>`/tools/${t.id}/`),...taxonomy.types.map(t=>`/types/${t.id}/`),...(await getPublished()).map(q=>`/questions/${q.id}/`)]:[];
