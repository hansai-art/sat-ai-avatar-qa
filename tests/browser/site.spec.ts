...
test('搜尋、篩選交集與 URL 還原',async({page})=>{
 ...
 await page.goto('questions/?q=429&chapter=ch02&sort=course');await expect(page.locator('#search-message')).toContainText('沒有符合的問題');
...
test('TG 別名及章節、工具、類型交叉篩選',async({page})=>{
 ...
 await page.reload();await expect(page.getByLabel('課程章節',{exact:true})).toHaveValue('ch01');await expect(page.locator('#active-filter-label')).toContainText('Hermes');
});
...
test('首頁搜尋片段、放寬條件保留關鍵字、返回搜尋',async({page})=>{
 ...
 await expect(page.locator('#result-list h2').first()).toContainText('429');
 // Returning to search must work before the detail page's external scripts load.
 await page.route('**/_astro/*.js',route=>route.abort());
 const searchURL=page.url();await page.locator('#result-list .question-card-link').first().click();
...
test('完整列表與搜尋結果皆將延伸問題排後，第一章小節網址保留',async({page})=>{
 ...
 await expect(page).toHaveURL(/lesson=ch01-03/);await expect(page.locator('input[name=lesson]')).toHaveValue('ch01-03');
});
test('手機首屏可看見第一題，複製按鈕靠近標題',async({page})=>{
 await page.goto('./');const first=page.locator('#starter-questions .question-card').first();await expect(first).toBeVisible();
 await page.evaluate(()=>document.fonts.ready);
 expect(await first.locator('h2').evaluate(el=>el.getBoundingClientRect().bottom)).toBeLessThan(page.viewportSize()!.height);
 await first.locator('.question-card-link').click();
